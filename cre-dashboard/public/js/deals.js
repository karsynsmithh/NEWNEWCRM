...
function attachDocListeners(dealId) {
  ...
  if (addBtn) {
    addBtn.addEventListener('click', async () => {
      ...
      const d = await API.get(`/api/deals/${dealId}`);
      document.getElementById('modal-body').innerHTML = await buildDocumentsTab(dealId, d);
      attachDocListeners(dealId);
    });
  }
}
...
function attachVendorListeners(dealId) {
  ...
  if (linkBtn) {
    linkBtn.addEventListener('click', async () => {
      ...
      document.getElementById('modal-body').innerHTML = await buildVendorsTab(dealId);
      attachVendorListeners(dealId);
    });
  }
}
...
function openDealModal(d = {}) {
  modal.open(d.id ? 'Edit Deal' : 'Add Deal', dealModalWithTabs(d, 'details'), async () => {
